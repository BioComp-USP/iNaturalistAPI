...
var IdentificationsController = class IdentificationsController {
  ...
  static search( req, callback ) {
    IdentificationsController.resultsForRequest( req, ( err, data ) => {
      ...
      shared.observations.preloadInto( data.results, localeOpts, err => {
        if( err ) { return callback( err ); }
  ...
  static reqToElasticQuery( req, callback ) {
    ...
    _.each([
      { http_param: "is_change", es_field: "taxon_change_id" }
    ], filter => {
  ...
  static similarSpecies( req, callback ) {
    ...
    Taxon.findByID( req.query.taxon_id, ( err, taxon ) => {
      ...
      IdentificationsController.speciesCounts( { query: idQuery, inat: req.inat }, ( err, rsp1 ) => {
        ...
        IdentificationsController.speciesCounts( { query: idQuery, inat: req.inat }, ( err, rsp2 ) => {
          if( err ) { return callback( err ); }
  ...
  static recentTaxa( req, callback ) {
    ...
    IdentificationsController.elasticResults( countReq, ( err, data ) => {
      ...
      ESModel.fetchBelongsTo( buckets, Identification, { }, function( err ) {
        if( err ) { return callback( err ); }
        var ids = _.map(buckets, b => b.identification );
        ESModel.fetchBelongsTo( ids, Observation, { }, function( err ) {
          if( err ) { return callback( err ); }
          var obs = _.map(buckets, b => b.identification.observation );
          shared.preloadMinimalForObservation( obs, localeOpts, err => {
            if( err ) { return callback( err ); }
            ESModel.fetchBelongsTo( ids, User, { }, err => {
              if( err ) { return callback( err ); }
              ESModel.fetchBelongsTo( buckets.concat(ids), Taxon, taxonOpts, function( err ) {
                if( err ) { return callback( err ); }
                buckets = _.reject( buckets, b => {
                  if( !b.taxon ) { return true; }
                  if( req.query.taxon_id &&
                      !_.contains( b.taxon.ancestor_ids, req.query.taxon_id ) ) {
                    return true;
                  }
                });
                callback( null, {
